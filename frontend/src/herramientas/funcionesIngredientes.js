...
export const manejarClickBusqueda = (busquedaInput, setBusqueda, setPaginaActual, ingredientes) => {
  const busquedaNormalizada = busquedaInput.trim().toLowerCase(); 
  //si el usuario introduce el ingrediente en mayus pasa a minus
  setBusqueda(busquedaInput);
  setPaginaActual(1);

  const resultados = ingredientes.filter((ingrediente) =>
    ingrediente.nombre.includes(busquedaNormalizada)
  );

  if (busquedaNormalizada.trim() !== "" && resultados.length === 0) {
    Swal.fire({
      icon: "warning",
